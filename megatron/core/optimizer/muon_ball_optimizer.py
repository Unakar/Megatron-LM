"""Megatron MuonBall optimizer wrapper."""
...
def get_megatron_muon_ball_optimizer(
    ...
    """Get the MuonBall optimizer for model chunks.

    MuonBall is a simplified version of Spectral Ball that fixes λ=0, which:
    - Eliminates the expensive bisection solver
    - Still maintains spectral norm constraints via retraction
    - Uses simple msign orthogonalization like Muon

    This function creates a chained optimizer where:
    - Linear weights (2D tensors) use MuonBall with spectral sphere constraints
    ...
    if config.use_distributed_optimizer:
        raise Exception('muon_ball with distributed optimizer is not supported.')
    ...
    log_single_rank(
        logger, logging.INFO, f'Setting up MuonBall optimizer with config {config}'
    )
    ...
    # This is because MuonBall already constrains weights to spectral sphere
    ...
    # Create MuonBall optimizer (enable QKV split and TP duplicated mode)
    muon_ball_optimizer = MuonBall(
        linear_param_groups,
        lr=config.lr,
        momentum_beta=config.muon_ball_momentum,
        use_nesterov=config.muon_ball_use_nesterov,
        ...
        power_iteration_steps=config.muon_ball_power_iteration_steps,
        msign_steps=config.muon_ball_msign_steps,
        radius_mode=config.muon_ball_radius_mode,
        scale_mode=config.muon_ball_scale_mode,
        retract_mode=config.muon_ball_retract_mode,
        retract_alpha=config.muon_ball_retract_alpha,
        split_qkv=config.muon_ball_split_qkv,
        is_qkv_fn=lambda p: getattr(p, 'is_qkv', False),
        qkv_split_shapes=tuple(qkv_split_shapes) if qkv_split_shapes is not None else None,
        qkv_split_mode=config.muon_ball_qkv_split_mode,
    ...
    muon_ball_optimizer.log_per_module_update_rms = config.log_per_module_update_rms
    if hasattr(muon_ball_optimizer, 'log_per_module_grad_rms'):
        muon_ball_optimizer.log_per_module_grad_rms = config.log_per_module_grad_rms

    # Save original optimizer name and switch to adam for the rest
    original_optimizer = config.optimizer
    config.optimizer = 'adam'

    # Define init state function for MuonBall
    def muon_ball_init_state_fn(opt, config=None):
        """Initialize MuonBall optimizer state for checkpointing.
    ...
    if config.fp16:
        raise Exception('muon_ball with fp16 is not supported.')
    ...
    if config.bf16:
        if layer_wise_distributed_optimizer:
            ...
            muon_ball_optimizer = Float16OptimizerWithFloat16Params(
                muon_ball_optimizer, config, None, muon_ball_init_state_fn
            )
    else:
        muon_ball_optimizer = FP32Optimizer(
            muon_ball_optimizer, config, muon_ball_init_state_fn
        )

    optimizers.append(muon_ball_optimizer)
    ...
    if layer_wise_distributed_optimizer:
        log_single_rank(
            logger, logging.INFO, 'Using LayerWiseDistributedOptimizer for MuonBall'
        )
        ...
        return LayerWiseDistributedOptimizer(
            ...
            config,
            pg_collection,
            init_state_fn_list=[muon_ball_init_state_fn, adam_init_state_fn],
        )
